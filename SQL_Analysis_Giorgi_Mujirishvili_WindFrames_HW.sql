...
WITH sales_report AS 
	(SELECT 
		...
		SUM(SUM(s.amount_sold)) OVER (PARTITION BY t.calendar_week_number,t.calendar_year 
			order by t.time_id RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cum_sum,
		ROUND(case 
			WHEN LOWER(t.day_name) = 'monday' then
				AVG(SUM(s.amount_sold)) OVER (ORDER  BY t.time_id ROWS BETWEEN 2 PRECEDING AND 1 FOLLOWING)
			WHEN LOWER(t.day_name) = 'friday' THEN
				AVG(SUM(s.amount_sold)) OVER (ORDER  BY t.time_id ROWS BETWEEN 1 PRECEDING AND 2 FOLLOWING)
			ELSE 
				AVG(SUM(s.amount_sold)) OVER (ORDER  BY t.time_id ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) END,2) AS centered_3_day_avg
	FROM sh.sales s
...
SELECT
	sr.*
FROM sales_report sr
WHERE sr.calendar_week_number BETWEEN 49 AND 51
ORDER BY sr.time_id;
...
WHERE c.country_region IN LOWER('americas', 'asia', 'europe')
